...
class CelebAHQ(data.Dataset):
    def __init__(self, root_dir,
        ...
        with open(valid_faces_file) as f:
            tmp_valid_faces = f.readlines()
        self.valid_faces = [x.strip() for x in tmp_valid_faces]
        ...
        # TODO: add comment
        self.inv = inv
        self.inv_data_dir = osp.join('datasets', 'real', 'inv', 'data')
        self.inv_codes_dir = osp.join('datasets', 'real', 'inv', 'latent_codes')
        ...
        self.images = []
        if filtered == True:            
            for filename in self.valid_faces:
                self.images.append(osp.join(self.data_dir, filename))
    ...
    def __getitem__(self, index):
        ...
        #           img_anon_code]
        output = [img_orig, img_orig_path, img_nn, img_nn_code, img_recon, img_recon_code, img_anon,
            img_anon_code]
